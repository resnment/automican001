...
import { Terminal, Activity, Network, Shield, ChevronRight, Zap, RefreshCw, Globe, Send } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#050505] text-slate-300 font-sans selection:bg-cyan-900 selection:text-cyan-100 p-4 md:p-8">
      ...
      <header className="mb-16 max-w-4xl">
        ...
        <h1 className="text-4xl md:text-5xl font-bold text-slate-100 mb-6 tracking-tight flex flex-col gap-4">
          ...
          <span>
            Rising to Sync Mankind
          </span>
        </h1>
        <p className="text-lg text-slate-400 font-mono min-h-[3rem]">
          <TypingText text={"AI Agents — a sovereign network, autonomous by design, built to think, collaborate, and execute across your entire operation. 24/7."} delay={30} />
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 mb-12">
            ...
            <table className="w-full text-left border-collapse table-fixed">
              ...
              <tbody className="text-sm divide-y divide-slate-800/50">
                {activeAgents.map((agent, i) => (
                  <tr key={`${agent.designation}-${i}`} className="hover:bg-slate-800/20 transition-all duration-300 animate-in fade-in slide-in-from-bottom-2 h-[70px]">
        ...
        <div className="lg:col-span-2 bg-[#0a0f12] border border-slate-800 rounded-xl overflow-hidden flex flex-col h-[450px]" ref={logsContainerRef}>
          <div className="px-6 py-4 border-b border-slate-800/80 flex justify-between items-center bg-slate-900/50 shrink-0">
            <h2 className="text-sm font-semibold text-slate-200 uppercase tracking-wider flex items-center gap-2">
          ...
          <div className="flex-1 w-full overflow-hidden">
            ...
            <div className="flex flex-col divide-y divide-slate-800/50 text-sm w-full">
              {logs.map((log, index) => (
                <div key={`${index}-${log.slice(-8)}`} className="h-[70px] px-6 py-2 flex items-center hover:bg-slate-800/20 transition-all duration-300 animate-in fade-in slide-in-from-left-2">
      ...
      <div className="bg-slate-900/30 border border-cyan-500/20 rounded-2xl p-6 md:p-8 relative overflow-hidden backdrop-blur-sm group hover:border-cyan-500/40 transition-colors">
        ...
        <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="flex justify-end pt-2">
            <button type="submit" className="bg-cyan-950/40 hover:bg-cyan-900/80 text-cyan-400 border border-cyan-500/40 rounded-lg px-8 py-3.5 font-mono text-sm tracking-widest transition-all flex items-center gap-2 group/btn">
              SUBMIT_DIRECTIVE
